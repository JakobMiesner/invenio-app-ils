...
from invenio_app_ils.permissions import backoffice_or_backoffice_readonly_permission
...
def search_filter_record_permissions():
    ...
    if (
        ...
        or backoffice_or_backoffice_readonly_permission().allows(g.identity)
    ):
        return dsl.Q()
...
def _filter_by_current_patron(search, query_string=None):
    ...
    if (
        has_request_context()
        and not backoffice_or_backoffice_readonly_permission().allows(g.identity)
    ):
